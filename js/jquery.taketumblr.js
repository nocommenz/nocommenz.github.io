 ...
 * @version 0.0.1
 * @requires jQuery
 * Custom by Piti Pitiaunchamroen : add callback when load data complete
 */
(function( $ ){
  var defaults = {
    api_key : null,
  ...
  var methods = {
    ...
    _getTumblrPosts: function() {
      var $this = this,
          ...
          $post_div,
          t_url = 'http://api.tumblr.com/v2/blog/'+data.settings.tumblr_host+'/posts?api_key='+data.settings.api_key+'&jsonp=?&callback=?&type='+data.settings.type+'&tag='+data.settings.tag+'&limit='+data.settings.limit,
          jxhr = [];
      ...
      $.getJSON(t_url, function(d) {
        ...
        $.each(tposts, function(index, value) {
          ...
          switch (value.type) {
            ...
            case 'text':
              t_title = value.title;
              // Custom by Piti : Edit right bound of substr to use option text_chars
              t_desc = value.body.substr(0, data.settings.text_chars) + '…';
              break;
            case 'audio':
            ...
            case 'video':
              t_title = $this.taketumblr('_strip',value.caption);
              t_desc = '<a href="'+value.permalink_url+'"><img src="'+value.thumbnail_url+'" width="100%"></a>';
              break;
            case 'photo':
          ...
          $.when.apply($, jxhr).done(function() {
            ...
            if (i + 1 == post_count) {
              ...
              $.each(tumblr_posts, function(i, v) {
                ...
                var p_popular = (v.popularity > 5) ? '<p class="clip-shares">'+v.fb_likes+' likes + '+v.tweets+' tweets + '+v.note_count+' notes = '+v.popularity+'</p>' : '';
                ...
                // Custom by Piti Pititaunchamroen
                if(typeof data.settings.callback == 'function') {
                  data.settings.callback($post_div, v);
                }
              });
            }
          });
